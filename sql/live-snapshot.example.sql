/*
Public-safe example: current reporting day, 24-hour scaffold, one row per source reading.

Replace placeholders only in a PRIVATE/local implementation.
Confirm the timezone semantics of [TIMESTAMP] before using this pattern.
*/
...
/*
Notes:
1. This preserves multiple readings within the same hour.
2. A NULL scaffold row means no source reading matched that hour.
3. If one hourly value is required, apply an explicit latest-timestamp rule.
4. If source timestamps are UTC, convert/bucket them consistently before deriving date/hour.
*/
